...
P = [0.5;0.5;0.4]; %position of COM of object in base frame
...
norm(t);
%% Force Closure algorithm implementation
comb = nchoosek(1:8,6);
Rnk=[];
for i=1:length(comb)
    W_sq(:,:,i)=W(:,comb(i,:));
...
W_sqfr = W_sq(:,:,Rnk);
w_t=zeros(6,length(W_sqfr));
for i = 1:length(W_sqfr)
    for j = 1:6
        w_t(:,i)=w_t(:,i)+W_sqfr(:,j,i);
    end
end
w_t=-w_t;
A_i=W(:,1:7);
syms n
%G=pinv(A_i)*w_t(:,1)+n*null(A_i)
%for i = 1:length(W_sqfr)
%k(:,i)=inv(W_sqfr(:,:,i))*w_t(:,i);
%end
%%
Tension=pinv(W)*w_t;
c87=nchoosek(1:8,7);
...
inv_W_i(:,:,i)=pinv(W_i(:,:,i));
rnk_W_i(i)=rank(W_i(:,:,i));
null_W_i(:,i)=null(W_i(:,:,i));
for i=1:16
lambda(:,i)=inv_W_i(:,:,end)*w_t(:,i);
end